...
import type { IRequest, ILog } from '../const'
...
function LoggerPanel() {
  ...
  const [networkRequests, setNetworkRequests] = useState<IRequest[]>([])
  const [consoleLogs, setConsoleLogs] = useState<ILog[]>([])
  ...
  useEffect(() => {
    const handleMessage = (message: { type: string; payload: IRequest | ILog }) => {
      if (message.type === 'NETWORK_REQUEST') {
        setNetworkRequests(prev => [...prev, message.payload as IRequest])
      }
    }
  ...
  useEffect(() => {
    ...
    const createLog = (type: ILog['type'], args: unknown[]): ILog => ({
  ...
  const getConsoleIcon = (type: ILog['type']): string => {
    switch (type) {
      case 'warn': return '⚠'
      case 'error': return '✕'
      case 'info': return 'ℹ'
